...
| `letterApi`     | `createDraft`、`updateDraft`、`submit`、`listPublic`、`detail`、`listMine`、`withdraw`、`delete`、`report`；`prepareImage`、`confirmImage`、`cancelImage`、`cleanupImages`、`imageUrls` | 图文草稿、提交、本人查询及图片授权/确认/清理已接线；公开列表和举报仍 `NOT_IMPLEMENTED`，真实平台待验；详见 [家书接口](letters.md) |
