# 云函数工程（TASK-101 / 103 / 200～202）

六个领域入口使用 `_shared/handler.ts` 的统一解析、鉴权、错误与请求日志。当前已实现 `authApi.login/getProfile/updateProfile` 及 `classApi.listSchools/listGrades/listClasses/selectClass/getCurrentClass`；`authApi.health` 仅用于开发/测试环境的数据库连通检查。资料更新和选班通过事务写入业务状态与审计。音频、家书、注销及具体管理业务仍返回 `NOT_IMPLEMENTED`。接口契约见 [云函数文档](../docs/cloud-functions.md) 与 [第二阶段说明](../docs/stage-2.md)；这些是本地代码与验证结果，尚未部署真实云环境。
...
- 后续真实资源操作必须先加载资源，再将其可信学校交给权限检查，不能只验证请求中的学校。仍为占位的音频、家书和管理入口不会读写业务资源。
...
`pagination.ts` 使用 HMAC 校验分页游标，包含稳定排序位置与服务端查询范围，拒绝跨查询复用、签名篡改和非法分页大小。签名密钥必须由服务端注入，至少 32 字节，不能在客户端或仓库设默认真实值。该 HMAC 工具保留供需要签名游标的业务使用。当前班级目录已经开放 `CursorPage<T>` 接口，采用版本化 Base64URL 游标绑定目录类型与学校 / 年级范围，按 `_id` 升序分页；它不使用 HMAC，游标也不作为身份或管理权限凭据。
